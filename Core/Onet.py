...
class Onet(Network):
    def _config(self):
        ...
        layer_factory.new_feed(name='input', layer_shape=(None, 48, 48, 3))
        layer_factory.new_conv(name='conv1', num_input_channels=3, filter_size=3,
                               num_output_channels=32, padding='VALID')
        layer_factory.new_max_pooling('pool1', kernel_size=3, strides=2, padding='SAME')
        layer_factory.new_conv(name='conv2', num_input_channels=32, filter_size=3,
                               num_output_channels=64, padding='VALID')
        layer_factory.new_max_pooling('pool2', kernel_size=3, strides=2, padding='VALID')
        layer_factory.new_conv(name='conv3', num_input_channels=64, filter_size=3,
                               num_output_channels=64, padding='VALID')
        layer_factory.new_max_pooling('pool3', kernel_size=2, strides=2, padding='VALID')
        layer_factory.new_conv(name='conv4', num_input_channels=64, filter_size=2,
                               num_output_channels=128, padding="VALID")

        layer_factory.new_full_connected(name='fc1', output_dim=256, relu=True)
        layer_factory.new_full_connected(name='fc2-1', output_dim=2, relu=False)
        layer_factory.new_softmax(name='softmax', axis=1)
        layer_factory.new_full_connected(name='fc2-2', output_dim=4, input_layername='fc1', relu=False)

    def _feed(self, image):
        return self._session.run(['onet/fc2-2/fc2-2:0', 'onet/softmax:0'], feed_dict={'onet/input:0': image})
    ...
    def detectbox(self, image, dets, vis=False):
        ...
        if len(keep_inds) > 0:
            ...
            return None, None
        keep = nms(boxes.copy(), 0.3, mode="Minimum")
        if len(keep) == 0:
            return None, None
